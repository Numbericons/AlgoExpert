...
function validateBst(tree, ancestors = {}, direction) {
  const leftVal = tree.left ? tree.left.value : -Infinity;
  const rightVal = tree.right ? tree.right.value : Infinity;
  let validDescendent = Object.keys(ancestors).length === 0;

  ancestors[tree.value] = tree.value;

  if (tree.value <= leftVal || tree.value > rightVal) return false;

  const validLeft = tree.left ? validateBst(tree.left, ancestors, direction ? direction : 'left') : true;
  const validRight = tree.right ? validateBst(tree.right, ancestors, direction ? direction : 'right') : true;

  if (direction) validDescendent = validateDescendent(direction, ancestors, tree.left, tree.right);

  return validLeft && validRight && validDescendent;
}
